...
JSON manifest stored at the exact target commit and a published GitHub Release
...
seals GitHub owner/repository, 40-hex base and target commits, manifest path and
SHA-256, published release tag and a plain-language approved scope. No URL is an
input.
...
target commit, manifest SHA-256 and approved-scope SHA-256. Validators compute
...
4. Manifest bytes and all identity fields must match sealed inputs.
